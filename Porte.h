...
public:
	...
	Porte(Couleur p_couleur, Piece* p_destination);

	//! Constructeur de copie
	Porte(const Porte & p_porte);

	//! Un destructeur qui ne fera rien
	~Porte();

	//! Surcharge de l'opérateur =
	const Porte & operator =(const Porte & p_source);

	//! Surcharge de l'opérateur ==
	bool operator ==(const Porte & p_source) const;
...
private:
	...
	Couleur m_couleur; /*!< Couleur de la porte.*/
